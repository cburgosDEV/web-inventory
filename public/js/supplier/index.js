...
/*!********************************************************!*\
  !*** ./resources/js/project_scripts/supplier/index.js ***!
  \********************************************************/
var vue = new Vue({
  el: '#index',
  components: {},
  ...
  methods: {
    ...
    showModal: function showModal() {
      var idSupplier = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
      this.clearData();
      if (idSupplier === 0) {
    ...
    initFormDetail: function initFormDetail(idSupplier, callback) {
      var _this3 = this;
      loading(true);
      var url = this.url + "/jsonDetail/" + idSupplier;
      window.axios.get(url).then(function (response) {
    ...
    softDelete: function softDelete(idSupplier) {
      var _this5 = this;
      this.$swal({
        icon: 'warning'
        ...
        if (result.value) {
          var context = _this5;
          _this5.initFormDetail(idSupplier, function () {
